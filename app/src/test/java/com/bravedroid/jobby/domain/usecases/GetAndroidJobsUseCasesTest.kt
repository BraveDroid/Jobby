...
class GetAndroidJobsUseCasesTest{
    ...
    fun `invoke android jobs uesCases success result`()= runBlockingTest{
        val jobsRepositoryMock = mock(JobsRepositoryImpl::class.java)
        ...
        val expected: GetAndroidJobsUseCase.GetAndroidJobsResponse = GetAndroidJobsUseCase.GetAndroidJobsResponse(listOf(remoteJob), listOf(nonRemoteJob))

        verify(jobsRepositoryMock).getAndroidJobs()
    ...
    fun `invoke android jobs uesCases error result `()= runBlockingTest{
        val jobsRepositoryMock = mock(JobsRepositoryImpl::class.java)
        `when`(jobsRepositoryMock.getAndroidJobs()).thenReturn(RuntimeException().toResultErrorUnknown().asFlow())
